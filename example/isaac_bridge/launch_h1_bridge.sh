#!/usr/bin/env bash
# launch_h1_bridge.sh — Start Isaac Sim + ROS2 bridge for H1 robot.
#
# Usage:
...
#   :15555  Isaac Sim → bridge  (state,    PUSH/PULL)
#   :15556  bridge → Isaac Sim  (commands, PUSH/PULL)
...
# ── 1. Isaac Sim process ──────────────────────────────────────────────────
echo "[launch] Starting Isaac Sim ..."
"${CONDA_PYTHON}" "${SCRIPT_DIR}/h1_isaac_sim.py" "$@" &
ISAAC_PID=$!
...
# Require CycloneDDS to match the control PC (both sides must use the same DDS vendor).
# Install on jqr if missing:  sudo apt install ros-humble-rmw-cyclonedds-cpp
...
if [[ -z "${RMW_SO}" ]]; then
    ...
    echo "[launch]        The control PC uses CycloneDDS; both sides must match."
    echo "[launch]        Fix: sudo apt install ros-humble-rmw-cyclonedds-cpp"
    echo "[launch]        Then re-run this script."
...
bash -c "
  ...
  python3 '${SCRIPT_DIR}/h1_isaac_ros2_bridge.py'
" &
BRIDGE_PID=$!
...
echo "[launch] Topics: /h1/lowstate (pub), /h1/lowcmd (sub), /h1/base_cmd (sub)"
wait "${ISAAC_PID}"
