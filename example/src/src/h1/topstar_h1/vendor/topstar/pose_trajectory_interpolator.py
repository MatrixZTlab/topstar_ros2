...
def rotation_distance(a: Rotation, b: Rotation) -> float:
...
def pose_distance(start_pose, end_pose):
    ...
    end_pos = end_pose[:3]
    start_rot = Rotation.from_rotvec(start_pose[3:6])
    end_rot = Rotation.from_rotvec(end_pose[3:6])
    pos_dist = np.linalg.norm(end_pos - start_pos)
    rot_dist = rotation_distance(start_rot, end_rot)
    return pos_dist, rot_dist

def joint_distance(start_pose, end_pose):
    start_pose = np.array(start_pose)
    end_pose = np.array(end_pose)
    start_pos = start_pose[:6]
    end_pos = end_pose[:6]
    pos_dist = np.linalg.norm(end_pos - start_pos)
    return pos_dist
...
class PoseTrajectoryInterpolator:
    def __init__(self, times: np.ndarray, poses: np.ndarray):
        ...
        if len(times) == 1:
            ...
            pos = poses[:, :3]
            rot = Rotation.from_rotvec(poses[:, 3:6])
            aux = poses[:, 6:]

            self.pos_interp = interp1d(times, pos,
                                          axis=0, assume_sorted=True)
            self.rot_interp = Slerp(times, rot)
            self.aux_interp = interp1d(times, aux,
                                          axis=0, assume_sorted=True)
...
class JointTrajectoryInterpolator:
    def __init__(self, times: np.ndarray, poses: np.ndarray):
        ...
        if len(times) == 1:
            ...
            self.single_step = False
            assert np.all(times[1:] >= times[:-1])
            self.pos_interp = interp1d(times, poses, axis=0, assume_sorted=True)
        self.dof = poses.shape[1]
    ...
    def trim(self,
        ...
        # interpolate
        all_poses = self(all_times)
        return JointTrajectoryInterpolator(times=all_times, poses=all_poses)

    def drive_to_waypoint(self,
                          joints, time, curr_time,
                          max_pos_speed=np.inf,
                          ) -> "JointTrajectoryInterpolator":
        assert (max_pos_speed > 0)
        time = max(time, curr_time)
        ...
        pos_dist = joint_distance(curr_joints, joints)
        pos_min_duration = pos_dist / max_pos_speed
        duration = time - curr_time
        ...
        trimmed_interp = self.trim(curr_time, curr_time)
        times = np.append(trimmed_interp.times, [last_waypoint_time], axis=0)
        poses = np.append(trimmed_interp.poses, [joints], axis=0)
    ...
    def schedule_waypoint(self,
        ...
        pos_dist = joint_distance(joints, end_joint)
        pos_min_duration = pos_dist / max_pos_speed
        duration = max(duration, pos_min_duration)
        assert duration >= 0
        last_waypoint_time = end_time + duration
