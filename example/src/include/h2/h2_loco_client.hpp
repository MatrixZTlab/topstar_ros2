#ifndef __TOPSTAR_ROBOT_H2_LOCO_CLIENT_HPP__
#define __TOPSTAR_ROBOT_H2_LOCO_CLIENT_HPP__
...
const int32_t ROBOT_API_ID_LOCO_GET_PHASE = 7006;
...
const int32_t ROBOT_API_ID_LOCO_SHAKE_HAND = 7214;
const int32_t ROBOT_API_ID_LOCO_WAVE_HAND = 7215;
...
class LocoClient {
 ...
 public:
  ...
  int32_t CallRaw(int32_t api_id, const std::string& json_param) {
    topstar_api::msg::Request req;
    req.header.identity.api_id = api_id;
    req.parameter = json_param;
    return base_client_.Call(req);
  }

  int32_t SetArmTask(int task_id) { return SetTaskId(task_id); }

  // High Level API
  int32_t Damp() { return SetFsmId(1); }
  int32_t Manual() { return SetFsmId(9); }
  ...
  int32_t WaveHand(bool turn_flag = false) {
    return SetTaskId(turn_flag ? 3 : 2);
  }

  int32_t WaveHandRaw(const std::string& json_param) {
    return CallRaw(ROBOT_API_ID_LOCO_WAVE_HAND, json_param);
  }
  ...
  int32_t ShakeHand(int stage = -1) {
    switch (stage) {
      ...
      default:
        ...
        return SetTaskId(first_shake_hand_stage_ ? 1 : 0);
    }
  }

  int32_t ShakeHandRaw(const std::string& json_param) {
    return CallRaw(ROBOT_API_ID_LOCO_SHAKE_HAND, json_param);
  }
...
#endif  // __TOPSTAR_ROBOT_H2_LOCO_CLIENT_HPP__
