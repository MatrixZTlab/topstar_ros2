...
class BaseClient {
  using Request = topstar_api::msg::Request;
  using Response = topstar_api::msg::Response;
 ...
 public:
  ...
  int32_t Call(Request req, nlohmann::json& js) {
    ...
    req.header.identity.id = topstar::common::GetSystemUptimeInNanoseconds();
    ...
    auto req_suber_ = node_->create_subscription<Response>(
        ...
        [&response_promise,
         identity_id](const std::shared_ptr<const Response> data) {
            std::cout << "Got response id=" << data->header.identity.id
              << " expected=" << identity_id
              << " status=" << data->header.status.code << std::endl;
